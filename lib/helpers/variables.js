...
function replaceVars(task, vars) {
  emitter.emit('stdout', 'Task: ' + JSON.stringify(task));
  emitter.emit('stdout', 'Vars: ' + JSON.stringify(vars));
  ...
  emitter.emit('stdout', 'Processed Task: ' + JSON.stringify(processedTask));
